...
export function useStorageAnalytics(storageData: any[]): StorageAnalytics {
  return useMemo(() => {
    ...
    storageData.forEach((d: any) => {
      const t = d.fileType || d.type || "Other";
...
export function useVersionAnalytics(versionsData: any[]): VersionAnalytics {
  return useMemo(() => {
    ...
    versionsData.forEach((v: any) => {
      const c = v.createdBy || "System";
      contrib[c] = (contrib[c] || 0) + 1;
    });
    const topContrib = Object.entries(contrib)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5);
    const latestVersion =
      versionsData.length > 0
        ? Math.max(...versionsData.map((v: any) => v.version || 1))
        : 0;
...
export function useOcrAnalytics(ocrData: any[]): OcrAnalytics {
  return useMemo(() => {
    ...
    ocrData.forEach((o: any) => {
      const l = o.language || "Unknown";
    ...
    const confidenceLevels = ocrData
      .filter((o: any) => o.confidence)
      .map((o: any) => parseFloat(o.confidence) || 0);
    const avgConfidence =
      confidenceLevels.length > 0
        ? confidenceLevels.reduce((a: number, b: number) => a + b, 0) /
          confidenceLevels.length
        : 0;
    const totalPages = ocrData.reduce(
      (s: number, o: any) => s + (o.pages || 1),
      0
    );
...
export function useAiAnalytics(aiSearchData: any[]): AiAnalytics {
  return useMemo(() => {
    const indexed = aiSearchData.filter(
      (a: any) => a.status === "Indexed"
    ).length;
    const notIndexed = aiSearchData.filter(
      (a: any) => a.status === "Not Indexed" || !a.status
    ).length;
    const confidenceLevels = aiSearchData
      .filter((a: any) => a.confidence)
      .map((a: any) => parseFloat(a.confidence) || 0);
    const avgConfidence =
      confidenceLevels.length > 0
        ? confidenceLevels.reduce((a: number, b: number) => a + b, 0) /
          confidenceLevels.length
        : 0;
    const typeDist: Record<string, number> = {};
    aiSearchData.forEach((a: any) => {
      const t = a.fileType || "Other";
      typeDist[t] = (typeDist[t] || 0) + 1;
    });
...
export function useSigAnalytics(signaturesData: any[]): SigAnalytics {
  return useMemo(() => {
    ...
    signaturesData.forEach((s: any) => {
      const sn = s.signerName || "Unknown";
      signerActivity[sn] = (signerActivity[sn] || 0) + 1;
    });
    const topSigners = Object.entries(signerActivity)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5);
    const signed = signaturesData.filter(
      (s: any) => s.status === "Signed"
    ).length;
    const pending = signaturesData.filter(
...
export function usePdfAnalytics(pdfData: any[]): PdfAnalytics {
  return useMemo(() => {
    const statusDist: Record<string, number> = {};
    pdfData.forEach((p: any) => {
      const s = p.status || "Draft";
      statusDist[s] = (statusDist[s] || 0) + 1;
    });
    const formatDist: Record<string, number> = {};
    pdfData.forEach((p: any) => {
      const f = p.format || "PDF";
      formatDist[f] = (formatDist[f] || 0) + 1;
    });
    const totalPages = pdfData.reduce(
      (s: number, p: any) => s + (p.pages || 0),
      0
    );
...
export function useDocumentKpis(
  ...
  return useMemo(() => {
    ...
    const pendingSigs = signaturesData.filter(
      (s: any) => s.status === "Pending"
    ).length;
    const signedDocs = signaturesData.filter(
      (s: any) => s.status === "Signed"
    ).length;
    const ocrCompleted = ocrData.filter(
